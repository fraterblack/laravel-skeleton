    ...
    <div class="field form-group col-md-6 col-lg-4">
        <select data-rule="empty" class="form-control" id="banner_place_id" name="banner_place_id">
            ...
            @foreach($places as $place)
                ...
                >{{ $place->name }} - {{ $place->width }}x{{ $place->height }}px</option>
            @endforeach
        </select>
    </div>
    <div class="field form-group col-md-6 col-lg-4">
        <button id="showBannerMap" class="btn btn-xs" href="{{ $place->present()->getUrlImage('original') }}" data-rel="lightbox"><span class="fa fa-search"></span> Ver Localização</button>
    </div>
</div>
...
<div id="bannerConfigInputsContainer" class="initially-hidden">
    <div class="row">
        ...
        <div class="form-group col-xs-12">
            @foreach($types as $code => $name)
                <label class="enabled-banner-type {{ $code }}">
    ...
    <div id="bannerImageInputs" class="ui segment initially-hidden">
        ...
        <div class="row">
            <div class="field form-group col-xs-12 col-md-6">
                {!! Form::label('url', 'URL') !!}
...
<div class="row">
    <div class="field required form-group col-xs-5 col-sm-3 col-md-2">
        {!! Form::label('created_at', 'Disponível A partir', ['data-title' => 'A partir desta data o banner começará a ser exibido', 'data-toggle' => 'tooltip']) !!}
        {!! Form::text('created_at', !empty($banner) ? $banner->present()->creationDate('d/m/Y H:i:s') : null, ['class' => 'form-control mask-datetime datetime-picker', 'data-rule' => 'empty', 'placeholder' => '__/__/____']) !!}
    </div>
</div>

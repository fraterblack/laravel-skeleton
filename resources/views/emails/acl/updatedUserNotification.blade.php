...
@section('email.content')
    ...
    Olá {{ $user->name }}!<br>
    seu cadastro como administrador do site foi alterado.<br><br>
    Segue seus dados para acesso ao painel administrativo:<br>
    <div>
        URL do Painel: <a href="{{ $panel_url }}">{{ $panel_url }}</a><br>
        E-mail: {{ $user->email }}
    @if(! empty($password))
        <br>Senha: <strong>{{ $password }}</strong> <em>(Recomendamos fortemente que você altere essa senha assim que se logar)</em>
    @endif
    </div>
    <br>
